...
function execWIthTimout(){
	...
	echo $1 $2 $3 $4 $5
	echo
	mvn -q exec:java -Dexec.mainClass="bgu.spl.mics.application.BookStoreRunner" -Dexec.args="$1 $2 $3 $4 $5"
}
...
runTestsForJsonFile(){
	...
	if [ $test_num == 1 ] ; then
		timeout 20s bash -c "execWIthTimout $args"
	elif [ $test_num -lt 5 ] ; then
		timeout 100s bash -c "execWIthTimout $args"
	elif [ $test_num -lt 9 ] ; then
		timeout 120s bash -c "execWIthTimout $args"
	else
		timeout 10s bash -c "execWIthTimout $args"
	fi
	...
	mvn -q compile; mvn -q exec:java -Dexec.mainClass="testRunner.Tester" -Dexec.args="$args"
...
if [ ! -f $tester_dir/automatic_check.csv ];then
	echo "GROUP_ID,FOLDER_STRUCTURE,TEST1,TEST2,TEST3,TEST4,TEST5,TEST6,TEST7,TEST8,TEST9" > $tester_dir/automatic_check.csv
fi
...
do
	...
	echo -e "$GROUP_ID,$FOLDER_STRUCTURE,$TEST1,$TEST2,$TEST3,$TEST4,$TEST5,$TEST6,$TEST7,$TEST8,$TEST9" >> $tester_dir/automatic_check.csv
	...
	mv $work_space/students/$olddir.tar.gz $work_space/checked_projects/
